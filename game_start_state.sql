...
-- ideologyEntropy is a concentration index (Herfindahl-based, no logarithms needed for SQL.js)
-- Higher entropy values indicate more ideological unity; lower values indicate diversity
CREATE VIEW country_power_metrics AS
WITH ideology_entropy AS (
    SELECT
        country_id,
        CASE
            WHEN SUM(percentage * percentage) < 1000 THEN 20.0 - (SUM(percentage * percentage) / 100.0)
            ELSE 10.0
        END AS ideology_entropy
...
-- Tech Tree system: tracks available research and progression
